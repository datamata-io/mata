...
class TestGGUFLoader:
    """Tests for .gguf detection and dispatch in UniversalLoader (v1.9.4)."""

    # ------------------------------------------------------------------
    # Extension detection
    # ------------------------------------------------------------------

    def test_detect_source_type_gguf_returns_local_file(self):
        """.gguf extension recognized as local_file by extension alone."""
        loader = UniversalLoader()
        source_type, resolved = loader._detect_source_type("vlm", "model.gguf")
        assert source_type == "local_file"
        assert resolved == "model.gguf"

    def test_detect_source_type_gguf_absolute_path(self, tmp_path):
        """.gguf absolute path (not on disk) still detected as local_file by extension."""
        loader = UniversalLoader()
        gguf_path = str(tmp_path / "clip-q8.gguf")
        source_type, _ = loader._detect_source_type("embed", gguf_path)
        assert source_type == "local_file"

    def test_detect_source_type_gguf_is_in_extension_whitelist(self):
        """Ensure .gguf is in the extension-based whitelist (not caught by HF slash-detection)."""
        loader = UniversalLoader()
        # A .gguf file without a slash should NOT become "huggingface"
        source_type, _ = loader._detect_source_type("classify", "local/model.gguf")
        assert source_type == "local_file"
    ...
    def test_load_from_file_gguf_vlm_dispatches_to_llamacpp_vlm_adapter(self, tmp_path):
        """_load_from_file with .gguf + task='vlm' → LlamaCppVLMAdapter + VLMWrapper."""
        ...
        with (
            patch(
                "mata.adapters.llamacpp_vlm_adapter.LlamaCppVLMAdapter",
                return_value=mock_adapter_inst,
            ) as mock_vlm_cls,
            patch(
                "mata.adapters.wrappers.vlm_wrapper.VLMWrapper",
                return_value=mock_wrapper,
            ) as mock_wrapper_cls,
        ):
            loader = UniversalLoader()
            result = loader._load_from_file("vlm", str(gguf))

        mock_vlm_cls.assert_called_once_with(model_path=str(gguf))
        mock_wrapper_cls.assert_called_once_with(mock_adapter_inst)
        assert result is mock_wrapper

    def test_load_from_file_gguf_embed_dispatches_to_llamacpp_embed_adapter(self, tmp_path):
        """_load_from_file with .gguf + task='embed' → LlamaCppEmbedAdapter + EmbedAdapter."""
        gguf = tmp_path / "clip.gguf"
        gguf.write_bytes(b"")

        mock_encoder = Mock()
        mock_embed_adapter = Mock()

        with (
            patch(
                "mata.adapters.llamacpp_embed_adapter.LlamaCppEmbedAdapter",
                return_value=mock_encoder,
            ) as mock_embed_cls,
            patch(
                "mata.adapters.embed_adapter.EmbedAdapter",
                return_value=mock_embed_adapter,
            ) as mock_embed_wrapper,
        ):
            loader = UniversalLoader()
            result = loader._load_from_file("embed", str(gguf))

        mock_embed_cls.assert_called_once_with(model_path=str(gguf))
        mock_embed_wrapper.assert_called_once_with(encoder=mock_encoder)
        assert result is mock_embed_adapter

    def test_load_from_file_gguf_classify_dispatches_to_llamacpp_classify_adapter(self, tmp_path):
        """_load_from_file with .gguf + task='classify' → LlamaCppClassifyAdapter."""
        gguf = tmp_path / "clip.gguf"
        gguf.write_bytes(b"")
        ...
        with patch(
            "mata.adapters.llamacpp_classify_adapter.LlamaCppClassifyAdapter",
            return_value=mock_adapter_inst,
        ) as mock_cls:
            loader = UniversalLoader()
            result = loader._load_from_file("classify", str(gguf))

        mock_cls.assert_called_once_with(model_path=str(gguf))
        assert result is mock_adapter_inst

    def test_load_from_file_gguf_detect_raises_unsupported_error(self, tmp_path):
        """task='detect' with .gguf raises UnsupportedModelError."""
        gguf = tmp_path / "model.gguf"
        gguf.write_bytes(b"")
        loader = UniversalLoader()
        with pytest.raises(UnsupportedModelError):
            loader._load_from_file("detect", str(gguf))

    def test_load_from_file_gguf_segment_raises_unsupported_error(self, tmp_path):
        """task='segment' with .gguf raises UnsupportedModelError."""
        gguf = tmp_path / "model.gguf"
        gguf.write_bytes(b"")
        loader = UniversalLoader()
        with pytest.raises(UnsupportedModelError):
            loader._load_from_file("segment", str(gguf))

    def test_load_from_file_gguf_error_message_lists_supported_tasks(self, tmp_path):
        """UnsupportedModelError for wrong task includes supported tasks in message."""
        gguf = tmp_path / "model.gguf"
        gguf.write_bytes(b"")
        loader = UniversalLoader()
        with pytest.raises(UnsupportedModelError, match="vlm.*embed.*classify|embed.*classify.*vlm"):
            loader._load_from_file("depth", str(gguf))

    # ------------------------------------------------------------------
    # ModelType.GGUF explicit routing
    # ------------------------------------------------------------------

    def test_load_with_explicit_type_gguf_routes_to_load_from_file(self, tmp_path):
        """model_type=ModelType.GGUF routes to _load_from_file."""
        from mata.core.types import ModelType
        ...
        loader = UniversalLoader()
        with patch.object(loader, "_load_from_file", return_value=mock_result) as mock_lff:
            result = loader._load_with_explicit_type("vlm", str(gguf), ModelType.GGUF)

        mock_lff.assert_called_once_with("vlm", str(gguf))
        assert result is mock_result

    def test_load_with_explicit_type_gguf_none_source_raises_model_not_found(self):
        """model_type=ModelType.GGUF with source=None → ModelNotFoundError."""
        from mata.core.types import ModelType

        loader = UniversalLoader()
        with pytest.raises(ModelNotFoundError, match="Valid GGUF file required"):
            loader._load_with_explicit_type("vlm", None, ModelType.GGUF)

    # ------------------------------------------------------------------
    # _validate_adapter_kwargs
    # ------------------------------------------------------------------

    def test_validate_adapter_kwargs_gguf_unknown_kwarg_emits_warning(self):
        """Unknown kwarg with ModelType.GGUF emits a warning."""
        from unittest.mock import patch

        import mata.core.model_loader as loader_mod
        from mata.core.types import ModelType

        loader = UniversalLoader()
        with patch.object(loader_mod.logger, "warning") as mock_warn:
            loader._validate_adapter_kwargs(ModelType.GGUF, unknown_extra_kwarg=True)
        assert mock_warn.called
        warning_text = mock_warn.call_args[0][0]
        assert "unknown_extra_kwarg" in warning_text

    def test_validate_adapter_kwargs_gguf_valid_kwargs_no_warning(self, caplog):
        """Known GGUF kwargs (n_gpu_layers, n_ctx, mmproj) produce no warning."""
        import logging

        from mata.core.types import ModelType

        loader = UniversalLoader()
        with caplog.at_level(logging.WARNING, logger="mata.core.model_loader"):
            loader._validate_adapter_kwargs(ModelType.GGUF, n_gpu_layers=4, n_ctx=1024, mmproj=None)
    ...
    def test_model_type_gguf_value_is_gguf_string(self):
        """ModelType.GGUF.value == 'gguf'."""
        from mata.core.types import ModelType

        assert ModelType.GGUF.value == "gguf"

    def test_model_type_normalize_gguf_string(self):
        """ModelType.normalize('gguf') returns ModelType.GGUF."""
        from mata.core.types import ModelType

        assert ModelType.normalize("gguf") is ModelType.GGUF

    def test_model_type_normalize_gguf_enum(self):
        """ModelType.normalize(ModelType.GGUF) returns ModelType.GGUF."""
        from mata.core.types import ModelType

        assert ModelType.normalize(ModelType.GGUF) is ModelType.GGUF
