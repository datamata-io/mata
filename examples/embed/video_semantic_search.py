...
def search(video_index: VideoIndex, query: str | list[str], model: str) -> None:
    ...
    all_matches = video_index.gallery.search_batch(text_embs, top_k=top_k, threshold=0.22) # adjust threshold as needed to balance precision vs recall

    for q, matches in zip(queries, all_matches):
        print(f"\nResults for: '{q}'")
        for i, m in enumerate(matches, 1):
            resolved = video_index.resolve_match(m)
            sm, ss = int(resolved.start_s) // 60, int(resolved.start_s) % 60
            em, es = int(resolved.end_s) // 60, int(resolved.end_s) % 60
...
if __name__ == "__main__":
    ...
    search(video_index, queries if len(queries) > 1 else queries[0], model)
