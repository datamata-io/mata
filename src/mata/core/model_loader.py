        ...
        if engine == "huggingface":
            ...
            def _adapt_torchvision_head_from_state_dict() -> None:
                ...
                if parsed_name in {"ssd300_vgg16", "ssdlite320_mobilenet_v3_large"}:
                    cls_head = adapter.model.head.classification_head
                    # SSDClassificationHead does not expose num_classes directly.
                    # Infer it from the regression head (out_channels = num_anchors * 4).
                    ...
                    _ssd_reg_conv = _ssd_last_conv(
                        next(iter(adapter.model.head.regression_head.module_list), None)
                    )
                    _ssd_cls_conv = _ssd_last_conv(
                        next(iter(cls_head.module_list), None)
                    )
                    if _ssd_reg_conv is not None and _ssd_cls_conv is not None:
                        old_num_classes = int(
                            _ssd_cls_conv.out_channels // max(_ssd_reg_conv.out_channels // 4, 1)
                        )
                    else:
                    ...
                    # Update num_columns (used by SSDScoringHead.forward() reshape).
                    if new_modules and _ssd_reg_conv is not None:
                        _new_first = _ssd_last_conv(new_modules[0])
                        if _new_first is not None:
                            cls_head.num_columns = _new_first.out_channels // max(
                                _ssd_reg_conv.out_channels // 4, 1
                            )

            _adapt_torchvision_head_from_state_dict()
            adapter.model.load_state_dict(state_dict)
            # Head replacement builds new modules on CPU by default.
            # Move the whole model back to adapter device for inference.
