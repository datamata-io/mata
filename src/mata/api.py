...
def train(
    ...
    **kwargs: Any,
    ...
    config = TrainingConfig(
        task=task, model=model, data=data, val_data=val_data,
        epochs=epochs, batch_size=batch_size, lr=lr, optimizer=optimizer,
        weight_decay=weight_decay, scheduler=scheduler, warmup_epochs=warmup_epochs,
        device=device, amp=amp, save_dir=save_dir, save_every=save_every,
        val_every=val_every, patience=patience, freeze_backbone=freeze_backbone,
        freeze_layers=freeze_layers, augment=augment, augment_config=augment_config,
        resume=resume, num_workers=num_workers, seed=seed, verbose=verbose,
    )
    config.validate()
...
def finetune(
    ...
    **kwargs: Any,
    ...
    """
    return train(
        task, model=model, data=data, val_data=val_data,
